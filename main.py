...
# --------------------------- Funciones -----------------------------------------------
def instrucciones():
    if not os.path.exists("datos/instrucciones.txt"):
        raise FileNotFoundError(f"El archivo {"datos/instrucciones.txt"} no existe.")

    with codecs.open("datos/instrucciones.txt", "r", encoding="utf-8") as f:
        fi = f.read()
    file = fi.split('\n')
...
# Momentaneamente no usamos bucles para poder ver claramente cada uno de los documentos
